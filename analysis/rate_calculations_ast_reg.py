...
#import measures
from study_definition_ast_reg import measures
...
for key, value in measures_dict.items():
    ...
    df = pd.read_csv(os.path.join(OUTPUT_DIR, 'joined',f'measure_{value.id}.csv'), parse_dates=['date']).sort_values(by='date')
    ...
    if value.id=='ast_reg_practice_rate':
        ...
        df_total.to_csv(os.path.join(OUTPUT_DIR, 'rate_table_total_ast_reg.csv'), index=False)
    ...
    #     child_code_table.to_csv('output/child_code_table.csv', index=False)
        ...
        plot_measures(
                df, 
                filename=f'plot_ast_reg_{value.group_by[0]}.png', 
                title=None, 
                column_to_plot='rate', 
        ...
        df.to_csv(os.path.join(OUTPUT_DIR, f'rate_table_ast_reg_{value.group_by[0]}.csv'), index=False)
